class Tenant::ClinicalDocumentationsController < Tenant::BaseController
  ...
  def show
    unless @clinical_documentation.file.attached?
      redirect_to tenant_clinical_documentations_path, alert: "Document file not found."
      return
    end
  ...
  def set_clinical_documentation
    @clinical_documentation = @current_organization.clinical_documentations.with_file.find(params[:id])
  end
  ...
  def load_filter_options
    @patients = @current_organization.patients.kept.order(:first_name, :last_name).limit(500)
    @providers = @current_organization.providers.kept.active.order(:last_name, :first_name)
