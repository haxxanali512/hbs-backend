class Admin::ClinicalDocumentationsController < Admin::BaseController
  ...
  def create
    ...
    @clinical_documentation.assign_attributes(clinical_documentation_params)
    ...
    if params[:section_names].present? && params[:section_contents].present?
      ...
      begin
        ...
        redirect_to admin_encounter_path(@encounter), alert: "Error: Invalid JSON content"
        return
      end
    end

    if @clinical_documentation.save
      redirect_to admin_encounter_path(@encounter), notice: "Clinical documentation created successfully."
    else
      redirect_to admin_encounter_path(@encounter), alert: "Error: #{@clinical_documentation.errors.full_messages.join(', ')}"
    end
  end
  ...
  def encounter_show_path
    admin_encounter_path(@encounter)
  end

  def update
    unless @clinical_documentation.draft?
      redirect_to admin_encounter_path(@encounter), alert: "Only draft documentation can be edited."
      return
    end
    ...
    if @clinical_documentation.update(clinical_documentation_params.except(:content_json))
      redirect_to admin_encounter_path(@encounter), notice: "Clinical documentation updated successfully."
    else
      redirect_to admin_encounter_path(@encounter), alert: "Error: #{@clinical_documentation.errors.full_messages.join(', ')}"
    end
  end
  ...
  def void_readonly
    ...
    unless current_user.super_admin? || current_user.hbs_user?
  ...
  def export_pdf
    unless @clinical_documentation.signed?
      redirect_to admin_encounter_path(@encounter), alert: "Only signed documentation can be exported."
      return
    end

    pdf_document = @clinical_documentation.rendered_pdf_document
    unless pdf_document
      redirect_to admin_encounter_path(@encounter), alert: "PDF not available."
      return
    end
