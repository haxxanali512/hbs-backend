class Admin::FeeScheduleItemsController < Admin::BaseController
  ...
  def new
    ...
    if params[:procedure_code_id].present?
      ...
      if procedure_code
        existing_item = @fee_schedule.organization.fee_schedule_item_for(procedure_code)
  ...
  def create
    ...
    if @fee_schedule_item.active? && @fee_schedule_item.procedure_code_id.present?
      existing_item = @fee_schedule.organization.fee_schedule_item_for(@fee_schedule_item.procedure_code)
      if existing_item && existing_item.id != @fee_schedule_item.id
        @fee_schedule_item.errors.add(:procedure_code, "FEE_DUP_ITEM - An active fee schedule item already exists for this procedure code in this organization.")
        @procedure_codes = ProcedureCode.order(:code)
