class Admin::PatientInsuranceCoveragesController < Admin::BaseController
  before_action :set_patient_insurance_coverage, only: [ :show, :edit, :update, :destroy, :activate, :terminate, :replace ]
  before_action :load_form_options, only: [ :index, :new, :edit, :create, :update ]

  def index
    @patient_insurance_coverages = PatientInsuranceCoverage.includes(:organization, :patient, :insurance_plan)
  ...
  def new
    @patient_insurance_coverage = PatientInsuranceCoverage.new(status: :draft, coverage_order: :primary)
    @patient_insurance_coverage.organization_id = params[:organization_id] if params[:organization_id].present?
  ...
  def create
    @patient_insurance_coverage = PatientInsuranceCoverage.new(patient_insurance_coverage_params)
    if @patient_insurance_coverage.save
      redirect_to admin_patient_insurance_coverage_path(@patient_insurance_coverage), notice: "Patient insurance coverage created."
    else
  ...
  def update
    if @patient_insurance_coverage.update(patient_insurance_coverage_params)
      redirect_to admin_patient_insurance_coverage_path(@patient_insurance_coverage), notice: "Patient insurance coverage updated."
    else
  ...
  def destroy
    if @patient_insurance_coverage.can_be_deleted?
      @patient_insurance_coverage.destroy
      redirect_to admin_patient_insurance_coverages_path, notice: "Coverage deleted."
    else
      redirect_to admin_patient_insurance_coverage_path(@patient_insurance_coverage), alert: "Cannot delete coverage: #{@patient_insurance_coverage.errors.full_messages.join(', ')}"
    end
  end

  def activate
    if @patient_insurance_coverage.activate!
      redirect_to admin_patient_insurance_coverage_path(@patient_insurance_coverage), notice: "Coverage activated."
    else
      redirect_to admin_patient_insurance_coverage_path(@patient_insurance_coverage), alert: "Cannot activate coverage: #{@patient_insurance_coverage.errors.full_messages.join(', ')}"
    end
  end
  ...
  def replace
    new_coverage_id = params[:new_coverage_id]
    unless new_coverage_id.present?
      redirect_to admin_patient_insurance_coverage_path(@patient_insurance_coverage), alert: "New coverage ID is required."
      return
    end

    if @patient_insurance_coverage.replace!(new_coverage_id: new_coverage_id)
      redirect_to admin_patient_insurance_coverage_path(@patient_insurance_coverage), notice: "Coverage replaced."
    else
      redirect_to admin_patient_insurance_coverage_path(@patient_insurance_coverage), alert: "Cannot replace coverage: #{@patient_insurance_coverage.errors.full_messages.join(', ')}"
    end
  end
  ...
  def set_patient_insurance_coverage
    @patient_insurance_coverage = PatientInsuranceCoverage.find(params[:id])
  end

  def load_form_options
    @organizations = Organization.where(activation_status: :activated).order(:name)
    @patients = Patient.active.order(:last_name, :first_name)
