...
class NotificationService
  class << self
    ...
    def notify_referral_partner_application_submitted(referral_partner)
      super_admin_users.each do |admin|
        Notification.create!(
          user: admin,
          organization: nil,
    ...
    def notify_referral_partner_application_approved(referral_partner, user:, approved_by:)
      Notification.create!(
        user: user,
        organization: nil,
        ...
        message: "Your referral partner access has been approved by #{approved_by.display_name}.",
        action_url: referral_portal_url_for("/dashboard")
      )
    end

    def notify_referral_attached_to_new_client(relationship)
      if (partner_user = relationship.referral_partner.user)
        Notification.create!(
          user: partner_user,
          ...
          action_url: referral_portal_url_for("/referrals/#{relationship.id}")
        )
      end
    ...
    def notify_referral_commission_generated(commission)
      partner_user = commission.referral_partner.user
      return unless partner_user

      Notification.create!(
        user: partner_user,
        organization: commission.referred_org,
        notification_type: Notification::NOTIFICATION_TYPES[:referral_commission_generated],
        title: "Commission Generated",
        message: "A new referral commission for #{commission.month.strftime('%B %Y')} is now available.",
        action_url: referral_portal_url_for("/payout_history")
      )
    end
    ...
    def notify_referral_payout_paid(commission)
      ...
      Notification.create!(
        ...
        message: "Your referral payout for #{commission.month.strftime('%B %Y')} has been marked paid.",
        action_url: referral_portal_url_for("/payout_history")
      )
    end
    ...
    def admin_portal_url_for(path)
      host = base_portal_host
      protocol = portal_protocol_for(host)
      "#{protocol}://admin.#{host}#{path}"
    end

    def referral_portal_url_for(path)
      host = base_portal_host
      protocol = portal_protocol_for(host)
      "#{protocol}://referral.#{host}#{path}"
    end
    ...
    def base_portal_host
      ...
      normalized_host = if host_parts.length >= 3 && %w[www admin referral].include?(host_parts.first)
