module PatientVisitStats
  ...
  VISIT_STATUSES = [ :ready_for_review, :reviewed, :ready_to_submit, :completed_confirmed, :sent ].freeze

  def encounters_entered_scope
    encounters.kept
  end

  # Encounters considered "visits" for reporting widgets.
  def eligible_visits_scope
    encounters_entered_scope.where(status: VISIT_STATUSES).where.not(date_of_service: nil)
  end

  def eligible_visits_count
    eligible_visits_scope.count
  end

  def encounters_entered_count
    encounters_entered_scope.count
  end
